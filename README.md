...
- a first bounded Cavenic Zombie Baseline MVP plus bounded natural-spawn and legacy-orb-drop follow-ups on top of that same cavenic line: `cavenic_zombie` now exists as a hostile foundation mob with legacy-mapped core attributes, a legacy texture on the vanilla zombie renderer path, a bounded CAVERN-only natural spawning follow-up and the legacy `1/8` `cavenic_orb` drop appended on top of the vanilla zombie loot baseline, documented in `docs/cavenic-zombie-baseline-mvp.md`, `docs/cavenic-zombie-natural-spawn-mvp.md` and `docs/cavenic-zombie-orb-drop-mvp.md`
