...
- a first bounded Cavenic Spider Baseline MVP plus bounded natural-spawn and legacy-orb-drop follow-ups as the fourth direct Cavenic mob foundation on top of that same cavenic line: `cavenic_spider` now exists as a hostile foundation mob with legacy-mapped core attributes, a legacy texture on the vanilla spider renderer path, a baseline spawn egg, a vanilla spider loot baseline, bounded CAVERN-only natural spawning and the legacy `1/8` `cavenic_orb` drop appended on top of that vanilla baseline, while legacy fall/fire damage behavior and special spider behavior remain intentionally out of scope for later bounded follow-ups; the bounded slices are documented in `docs/cavenic-spider-baseline-mvp.md`, `docs/cavenic-spider-natural-spawn-mvp.md` and `docs/cavenic-spider-orb-drop-mvp.md`
