...
- a first bounded Cavenic Zombie Baseline MVP plus a bounded Cavenic Zombie Natural Spawn MVP follow-up on top of that same cavenic line: `cavenic_zombie` now exists as a hostile foundation mob with legacy-mapped core attributes, a legacy texture on the vanilla zombie renderer path and a bounded CAVERN-only natural spawning follow-up, documented in `docs/cavenic-zombie-baseline-mvp.md` and `docs/cavenic-zombie-natural-spawn-mvp.md`
