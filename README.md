...
- a first bounded Cavenic Creeper Baseline MVP plus bounded natural-spawn, legacy-orb-drop and legacy-damage-behavior follow-ups as the third direct Cavenic mob foundation on top of that same cavenic line: `cavenic_creeper` now exists as a hostile foundation mob with legacy-mapped core attributes, a legacy texture on the vanilla creeper renderer path, a baseline spawn egg, bounded CAVERN-only natural spawning, the legacy `1/5` `cavenic_orb` drop appended on top of the vanilla creeper loot baseline and the legacy fall-damage reduction and fire-damage immunity behavior, while legacy fuse/explosion tuning and custom explosion behavior remain intentionally out of scope for later bounded follow-ups; the bounded slices are documented in `docs/cavenic-creeper-baseline-mvp.md`, `docs/cavenic-creeper-natural-spawn-mvp.md`, `docs/cavenic-creeper-orb-drop-mvp.md` and `docs/cavenic-creeper-damage-behavior-mvp.md`
