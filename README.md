...
- a first bounded Cavenic Bow Baseline MVP plus mode-state follow-up on top of that cavenic material line: `cavenic_bow` now exists as a repairable vanilla-baseline bow with stack-local mode state and sneak-use mode cycling, while custom arrows and other special bow behavior are intentionally left for later follow-ups; the bounded slices are documented in `docs/cavenic-bow-baseline-mvp.md` and `docs/cavenic-bow-mode-state-mvp.md`
