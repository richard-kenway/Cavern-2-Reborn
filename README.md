...
- a first bounded Cavenic Bow Baseline MVP on top of that cavenic material line: `cavenic_bow` now exists as a repairable vanilla-baseline bow with legacy mode cycling, custom arrows and other special bow behavior intentionally left for a later follow-up; the bounded slice is documented in `docs/cavenic-bow-baseline-mvp.md`
