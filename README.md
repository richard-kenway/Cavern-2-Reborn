...
- a Crazy Spider Baseline MVP plus bounded legacy-loot follow-up as the fourth crazy-variant foundation follow-up on top of that same roster: `crazy_spider` now exists as a `Spider`-based hostile baseline with legacy-mapped core attributes, a legacy texture on the vanilla spider renderer path, a baseline spawn egg and a vanilla spider loot baseline, with the inspected legacy `1500.0` max-health request explicitly clamped by the modern `generic.max_health` runtime ceiling of `1024.0`, while legacy crazy-roster natural spawning stays deferred because the source still routes Crazy Spider through the old Cavenia-only provider/config branch rather than the normal `CAVERN` biome-spawn path, the inherited legacy `1/8` `cavenic_orb` drop is now restored explicitly on top of that same vanilla spider loot baseline, and the direct blindness/poison combat hooks plus inherited damage, boss and particle branches remain intentionally deferred until each branch is inspected and restored honestly; the bounded slices are documented in `docs/crazy-spider-baseline-mvp.md` and `docs/crazy-spider-loot-mvp.md`
