...
- a first bounded Cavenic Creeper Baseline MVP as the third direct Cavenic mob foundation on top of that same cavenic line: `cavenic_creeper` now exists as a hostile foundation mob with legacy-mapped core attributes, a legacy texture on the vanilla creeper renderer path, a baseline spawn egg and the vanilla creeper loot-table baseline, while natural spawning, the legacy `1/5` `cavenic_orb` drop, the legacy fall/fire damage behavior and custom explosion behavior remain intentionally out of scope for later bounded follow-ups; the bounded slice is documented in `docs/cavenic-creeper-baseline-mvp.md`
