...
- a first bounded Cavenic Bow Baseline MVP plus mode-state, Snipe and Rapid follow-ups on top of that cavenic material line: `cavenic_bow` now exists as a repairable vanilla-baseline bow with stack-local mode state, sneak-use mode cycling, a bounded full-charge Snipe boost on vanilla arrows and a bounded Rapid power ramp on vanilla arrows, while TORCH, custom projectile entities and other special bow behavior are intentionally left for later follow-ups; the bounded slices are documented in `docs/cavenic-bow-baseline-mvp.md`, `docs/cavenic-bow-mode-state-mvp.md`, `docs/cavenic-bow-snipe-mode-mvp.md` and `docs/cavenic-bow-rapid-mode-mvp.md`
