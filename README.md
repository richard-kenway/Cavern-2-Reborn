...
- The same runtime smoke slice now also covers `crazy_skeleton` runtime registry/attribute/spawn-egg smoke, the explicit no-natural-spawn baseline boundary, the vanilla skeleton loot-table baseline and reflection coverage that the baseline still does not add Crazy Skeleton damage, boss, particle or `EntityAIAttackCavenicBow` follow-up overrides.
