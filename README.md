...
- a first bounded Cavenic Zombie Baseline MVP on top of that same cavenic line: `cavenic_zombie` now exists as a spawn-egg-driven hostile foundation mob with legacy-mapped core attributes, a legacy texture on the vanilla zombie renderer path and no natural spawning yet; the bounded slice is documented in `docs/cavenic-zombie-baseline-mvp.md`
