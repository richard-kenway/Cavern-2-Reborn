...
- a first bounded Cavenic Bear Baseline MVP plus bounded natural-spawn and legacy-damage-behavior follow-ups as the sixth direct Cavenic mob foundation on top of that same cavenic line: `cavenic_bear` now exists as a `PolarBear`-based hostile foundation mob with legacy-mapped core attributes, a legacy texture on the vanilla polar bear renderer path, a baseline spawn egg, a vanilla polar bear loot baseline, bounded CAVERN-only natural spawning and the legacy fall-damage reduction and fire-damage immunity behavior, while the inspected legacy bear loot line remains explicitly absent because `EntityCavenicBear` never overrides `dropLoot(...)`, and anger rewrites, taming/riding behavior and other special bear behavior remain intentionally out of scope for later bounded follow-ups; the bounded slices are documented in `docs/cavenic-bear-baseline-mvp.md`, `docs/cavenic-bear-natural-spawn-mvp.md`, `docs/cavenic-bear-loot-absent-or-deferred.md` and `docs/cavenic-bear-damage-behavior-mvp.md`
