...
- a first bounded Cavenic Bow Baseline MVP plus mode-state and Snipe follow-ups on top of that cavenic material line: `cavenic_bow` now exists as a repairable vanilla-baseline bow with stack-local mode state, sneak-use mode cycling and a bounded full-charge Snipe boost on vanilla arrows, while RAPID, TORCH, custom projectile entities and other special bow behavior are intentionally left for later follow-ups; the bounded slices are documented in `docs/cavenic-bow-baseline-mvp.md`, `docs/cavenic-bow-mode-state-mvp.md` and `docs/cavenic-bow-snipe-mode-mvp.md`
