...
- a first bounded Cavenic Bear Baseline MVP plus bounded natural-spawn follow-up as the sixth direct Cavenic mob foundation on top of that same cavenic line: `cavenic_bear` now exists as a `PolarBear`-based hostile foundation mob with legacy-mapped core attributes, a legacy texture on the vanilla polar bear renderer path, a baseline spawn egg, a vanilla polar bear loot baseline and bounded CAVERN-only natural spawning, while custom loot, fall/fire damage behavior, anger rewrites, taming/riding behavior and other special bear behavior remain intentionally out of scope for later bounded follow-ups; the bounded slices are documented in `docs/cavenic-bear-baseline-mvp.md` and `docs/cavenic-bear-natural-spawn-mvp.md`
