...
- a consolidated Cavenia Crazy Roster Natural Spawn Boundary MVP on top of those four crazy baselines: the inspected legacy `CaveEntityRegistry.CRAZY_SPAWNS` roster contains only `crazy_skeleton`, `crazy_creeper`, `crazy_zombie` and `crazy_spider`, each with weight `1` and group size `1 / 1`, and the inspected `WorldProviderCavenia#createSpawnCreature(...)` path only switches to that roster through the old Cavenia-only `crazySpawnChance` plus nearby non-boss `ICavenicMob` exclusion branch rather than the normal `CAVERN` biome-spawn path, so current Reborn still does not add fake normal `CAVERN` crazy biome modifiers, biome tags or spawn placements; the shared boundary is documented in `docs/cavenia-crazy-roster-natural-spawn-boundary.md`
