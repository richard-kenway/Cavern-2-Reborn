...
- a first bounded Cavenic Creeper Baseline MVP plus bounded natural-spawn and legacy-orb-drop follow-ups as the third direct Cavenic mob foundation on top of that same cavenic line: `cavenic_creeper` now exists as a hostile foundation mob with legacy-mapped core attributes, a legacy texture on the vanilla creeper renderer path, a baseline spawn egg, bounded CAVERN-only natural spawning and the legacy `1/5` `cavenic_orb` drop appended on top of the vanilla creeper loot baseline, while the legacy fall/fire damage behavior and custom explosion behavior remain intentionally out of scope for later bounded follow-ups; the bounded slices are documented in `docs/cavenic-creeper-baseline-mvp.md`, `docs/cavenic-creeper-natural-spawn-mvp.md` and `docs/cavenic-creeper-orb-drop-mvp.md`
