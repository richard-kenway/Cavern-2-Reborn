...
- a first bounded Cavenic Bear Baseline MVP as the sixth direct Cavenic mob foundation on top of that same cavenic line: `cavenic_bear` now exists as a `PolarBear`-based hostile foundation mob with legacy-mapped core attributes, a legacy texture on the vanilla polar bear renderer path, a baseline spawn egg and a vanilla polar bear loot baseline, while natural spawning, custom loot, fall/fire damage behavior and special bear behavior remain intentionally out of scope for later bounded follow-ups; the bounded slice is documented in `docs/cavenic-bear-baseline-mvp.md`
