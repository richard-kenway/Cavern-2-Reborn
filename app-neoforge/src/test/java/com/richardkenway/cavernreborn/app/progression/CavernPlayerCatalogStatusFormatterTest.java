...
class CavernPlayerCatalogStatusFormatterTest {
    ...
    void formatsLockedCatalogForNewPlayer() {
        ...
        assertEquals(
            "CAVERN catalog for TestPlayer: rank=novice, entries=apprentice_supply_cache [reward, locked: requires apprentice, current novice, grants torch x16, bread x8]; torch_supply [service, locked: requires apprentice, current novice, grants torch x16]",
    ...
    void formatsAvailableAndCooldownCatalogStates() {
        ...
        assertEquals(
            "CAVERN catalog for TestPlayer: rank=apprentice, entries=apprentice_supply_cache [reward, available: use /cavern use apprentice_supply_cache, grants torch x16, bread x8]; torch_supply [service, available: use /cavern use torch_supply, grants torch x16]",
            CavernPlayerCatalogStatusFormatter.format(
                "TestPlayer",
                snapshot,
                List.of(
