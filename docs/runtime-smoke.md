...
- cavenia runtime biome-source collect-possible-biomes holder-set builder consolidation and next-decision smoke
- cavenia runtime biome-source runtime construction/factory go-no-go smoke
...
The same runtime smoke slice now also covers the Cavenia Runtime BiomeSource Selector-to-Weighted-Candidate Bridge Go/No-Go MVP: `CaveniaRuntimeBiomeSourceSelectorToWeightedCandidateBridgeGoNoGoDecision` now resolves from `app-neoforge`, selected decision is `PROCEED_WITH_SELECTOR_TO_WEIGHTED_CANDIDATE_BRIDGE_READINESS_NEXT`, decision is go for selector-to-weighted-candidate bridge readiness next, decision is readiness-only for the next slice, selector-to-weighted-candidate bridge readiness is not implemented, selector-to-weighted-candidate bridge implementation is not ready, selector-to-weighted-candidate bridge runtime is not ready, guardrail count is `19`, all guardrails are enforced in this slice, allowed next-slice readiness action count is `5`, allowed next-slice bridge implementation action count is `0`, allowed next-slice runtime action count is `0`, no bridge implementation or runtime action is allowed by guardrails, next slice may add only selector-to-weighted-candidate bridge readiness data and may pin only selector-input surface, weighted-selector query, adapter query and candidate-key output policy, next slice may not implement selector-to-weighted-candidate bridge, may not call weighted selector with derived input, may not call adapter with derived input and may not wire into `getNoiseBiome(...)`, selector-input algorithm consolidation remains ready, selector-input algorithm remains ready, weighted selector compatibility remains ready, adapter compatibility remains ready, runtime biome resolution, registration, activation and `EntityCaveman -> deferred:caveman` remain blocked, and the selector-to-weighted-candidate bridge go/no-go note at `docs/cavenia-runtime-biome-source-selector-to-weighted-candidate-bridge-go-no-go-mvp.md` documents that inert path.

The same runtime smoke slice now also covers the Cavenia Runtime BiomeSource Selector-to-Weighted-Candidate Bridge Readiness MVP: `CaveniaRuntimeBiomeSourceSelectorToWeightedCandidateBridgeReadiness` now resolves from `app-neoforge`, selector-to-weighted-candidate bridge readiness is ready, selector input source is pinned, weighted selector source is pinned, adapter source is pinned, candidate inventory source is pinned, selector input shape is pinned, total weight contract is pinned, normalization policy is pinned, candidate key output shape is pinned, candidate entry count is pinned, fallback policy dependency is pinned, possible-biomes constraint is pinned and noise-biome selection dependency is pinned, bridge implementation is not ready, selector or adapter calls are not ready, runtime wiring is not ready and the real selector-to-weighted-candidate bridge is still deferred, selector input algorithm is ready and selector input algorithm runtime is not ready, sample derivations `deriveSelectorInput(0, 0, 0) == -104499101`, `deriveSelectorInput(1, 2, 3) == 700186390` and `deriveSelectorInput(-16, 64, 16) == -1039724642` remain pinned, weighted selector compatibility is ready, weighted selection total weight remains `675`, adapter compatibility is ready and adapter runtime is not ready, candidate entry count remains `14`, candidate inventory is ready and candidate keys are string-only, fallback legacy biome name is `PLAINS`, fallback candidate modern biome key is `minecraft:plains`, bridge go/no-go decision is ready and bridge go/no-go runtime is not ready, selector-input algorithm consolidation is ready and selector-input algorithm consolidation runtime is not ready, selector-input algorithm selected next decision matches, implementation go/no-go decision is ready and implementation go/no-go runtime is not ready, readiness-chain consolidation is ready and readiness-chain runtime is not ready, all current readiness layers are ready, runtime-ready layer count remains `0` and consolidated readiness layer count remains `6`, holder conversion readiness is ready and holder conversion runtime is not ready, holder resolution is not ready, resource-location conversion is not ready and resource-key conversion is not ready, possible-biomes readiness is ready and possible-biomes runtime is not ready, fallback policy readiness is ready and fallback policy runtime is not ready, codec method shape stub is ready and codec method shape runtime is not ready, designated subclass remains ready and designated subclass runtime is not ready, runtime holder return is not ready, collectPossibleBiomes implementation is not ready, getNoiseBiome implementation is not ready, usable runtime biome source is not ready, registry lookup access is not ready, usable codec implementation is not ready, codec is not registered and biome-source type is not registered, normal runtime construction is not allowed, unsupported method stubs only remain in place, selected-surface readiness item count remains `68`, global readiness matrix total requirement count remains `46`, global readiness matrix blocked requirement count remains `46`, dimension JSON is absent, dimension type JSON is absent, Caveman remains deferred, `CAVENIA_LEVEL_KEY` still resolves to `cavernreborn:cavenia`, `level.getServer().getLevel(CAVENIA_LEVEL_KEY) == null`, no `dimension/cavenia.json` or `dimension_type/cavenia.json` exists, no Cavenia configured or placed feature resources, biome modifiers, biome tags or other worldgen resources exist, direct Cavenic and crazy boundaries remain unchanged, and the selector-to-weighted-candidate bridge readiness note at `docs/cavenia-runtime-biome-source-selector-to-weighted-candidate-bridge-readiness-mvp.md` documents that inert path.

The same runtime smoke slice now also covers the Cavenia Runtime BiomeSource Selector-to-Weighted-Candidate Bridge Implementation Go/No-Go MVP: `CaveniaRuntimeBiomeSourceSelectorToWeightedCandidateBridgeImplementationGoNoGoDecision` now resolves from `app-neoforge`, selected decision is `PROCEED_WITH_PURE_NON_RUNTIME_SELECTOR_TO_WEIGHTED_CANDIDATE_BRIDGE_NEXT`, decision is go for pure non-runtime bridge next, selector-to-weighted-candidate bridge is not implemented in this slice, selector-to-weighted-candidate bridge runtime is not ready, guardrail count is `17`, all guardrails are enforced in this slice, allowed next-slice pure bridge action count is `5`, allowed next-slice runtime action count is `0`, no runtime action is allowed by guardrails, next slice may add pure non-runtime selector-to-weighted-candidate bridge, may use signed int selector input, may use existing weighted selector or adapter, may return string candidate key and may use candidate inventory data, next slice may not import Minecraft runtime APIs and may not wire into `getNoiseBiome(...)`, bridge readiness is ready and bridge readiness runtime is not ready, bridge implementation is not ready and bridge runtime is not ready, selector input output shape is `signed int selector input`, candidate key output shape is `string modern biome candidate key`, weighted selection total weight remains `675`, candidate entry count remains `14`, candidate inventory is ready and candidate keys are string-only, weighted selector compatibility is ready, adapter compatibility is ready and adapter runtime is not ready, selector input algorithm is ready and selector input algorithm runtime is not ready, sample derivations `deriveSelectorInput(0, 0, 0) == -104499101`, `deriveSelectorInput(1, 2, 3) == 700186390` and `deriveSelectorInput(-16, 64, 16) == -1039724642` remain pinned, previous bridge go/no-go decision is ready and previous bridge go/no-go runtime is not ready, selector-input algorithm consolidation is ready and selector-input algorithm consolidation runtime is not ready, readiness-chain consolidation is ready and readiness-chain runtime is not ready, all current readiness layers are ready, runtime-ready layer count remains `0` and consolidated readiness layer count remains `6`, holder conversion readiness is ready and holder conversion runtime is not ready, holder resolution is not ready, resource-location conversion is not ready and resource-key conversion is not ready, possible-biomes readiness is ready and possible-biomes runtime is not ready, fallback policy readiness is ready and fallback policy runtime is not ready, codec method shape stub is ready and codec method shape runtime is not ready, designated subclass remains ready and designated subclass runtime is not ready, runtime holder return is not ready, collectPossibleBiomes implementation is not ready, getNoiseBiome implementation is not ready, usable runtime biome source is not ready, registry lookup access is not ready, usable codec implementation is not ready, codec is not registered and biome-source type is not registered, normal runtime construction is not allowed, unsupported method stubs only remain in place, selected-surface readiness item count remains `68`, global readiness matrix total requirement count remains `46`, global readiness matrix blocked requirement count remains `46`, dimension JSON is absent, dimension type JSON is absent, Caveman remains deferred, `CAVENIA_LEVEL_KEY` still resolves to `cavernreborn:cavenia`, `level.getServer().getLevel(CAVENIA_LEVEL_KEY) == null`, no `dimension/cavenia.json` or `dimension_type/cavenia.json` exists, no Cavenia configured or placed feature resources, biome modifiers, biome tags or other worldgen resources exist, direct Cavenic and crazy boundaries remain unchanged, and the selector-to-weighted-candidate bridge implementation go/no-go note at `docs/cavenia-runtime-biome-source-selector-to-weighted-candidate-bridge-implementation-go-no-go-mvp.md` documents that inert path.

The same runtime smoke slice now also covers the Cavenia Runtime BiomeSource Selector-to-Weighted-Candidate Bridge Pure Non-Runtime Implementation MVP: `CaveniaRuntimeBiomeSourceSelectorToWeightedCandidateBridge` now resolves from `app-neoforge`, selector-to-weighted-candidate bridge is ready, pure non-runtime bridge is ready, signed int selector input is ready, weighted selector or adapter path is ready, string candidate key output is ready, candidate inventory compatibility is ready, normalization is delegated to selector path, deterministic bridge output is ready, Minecraft runtime API dependency is absent, registry or holder dependency is absent, fallback runtime usage is absent, getNoiseBiome wiring is not ready, runtime selection is not ready and real runtime selection is still deferred, sample origin candidate key `minecraft:taiga`, sample mixed candidate key `minecraft:desert` and sample negative candidate key `minecraft:desert` are non-blank and exist in inventory, sample keys exist in inventory, bridge implementation go/no-go decision is ready and bridge implementation go/no-go runtime is not ready, bridge readiness is ready and bridge readiness runtime is not ready, selector input algorithm is ready and selector input algorithm runtime is not ready, sample derivations `deriveSelectorInput(0, 0, 0) == -104499101`, `deriveSelectorInput(1, 2, 3) == 700186390` and `deriveSelectorInput(-16, 64, 16) == -1039724642` remain pinned, weighted selector compatibility is ready, weighted selection total weight remains `675`, adapter compatibility is ready and adapter runtime is not ready, candidate entry count remains `14`, candidate inventory is ready and candidate keys are string-only, fallback policy readiness is ready and fallback policy runtime is not ready, possible-biomes readiness is ready and possible-biomes runtime is not ready, noise-biome selection readiness is ready and noise-biome selection runtime is not ready, holder conversion readiness is ready and holder conversion runtime is not ready, holder resolution is not ready, resource-location conversion is not ready and resource-key conversion is not ready, codec method shape stub is ready and codec method shape runtime is not ready, designated subclass remains ready and designated subclass runtime is not ready, runtime holder return is not ready, collectPossibleBiomes implementation is not ready, getNoiseBiome implementation is not ready, usable runtime biome source is not ready, registry lookup access is not ready, usable codec implementation is not ready, codec is not registered and biome-source type is not registered, normal runtime construction is not allowed, unsupported method stubs only remain in place, selected-surface readiness item count remains `68`, global readiness matrix total requirement count remains `46`, global readiness matrix blocked requirement count remains `46`, dimension JSON is absent, dimension type JSON is absent, Caveman remains deferred, `CAVENIA_LEVEL_KEY` still resolves to `cavernreborn:cavenia`, `level.getServer().getLevel(CAVENIA_LEVEL_KEY) == null`, no `dimension/cavenia.json` or `dimension_type/cavenia.json` exists, no Cavenia configured or placed feature resources, biome modifiers, biome tags or other worldgen resources exist, direct Cavenic and crazy boundaries remain unchanged, and the pure non-runtime selector bridge note at `docs/cavenia-runtime-biome-source-selector-to-weighted-candidate-bridge-pure-non-runtime-implementation-mvp.md` documents that inert path.

The same runtime smoke slice now also covers the Cavenia Runtime BiomeSource Selector-to-Weighted-Candidate Bridge Consolidation / Next Decision MVP: `CaveniaRuntimeBiomeSourceSelectorToWeightedCandidateBridgeConsolidation` now resolves from `app-neoforge`, selector-to-weighted-candidate bridge consolidation is ready, selector-to-weighted-candidate bridge is ready and selector-to-weighted-candidate bridge runtime is not ready, pure non-runtime bridge is ready, signed int selector input is ready, weighted selector or adapter path is ready, string candidate key output is ready, candidate inventory compatibility is ready, sample candidate keys are verified and candidate keys remain string-only, selected next decision is `PROCEED_WITH_CANDIDATE_KEY_TO_HOLDER_CONVERSION_GO_NO_GO_NEXT`, candidate-key-to-holder conversion go/no-go is next, candidate-key-to-holder conversion readiness is ready, candidate-key-to-holder conversion implementation is not ready and candidate-key-to-holder conversion runtime is not ready, the next slice may add only a candidate-key-to-holder conversion go/no-go decision and may not add candidate-key-to-holder conversion readiness or implementation, registry lookup access, holder resolution, resource-location conversion, resource-key conversion or `getNoiseBiome(...)` wiring, candidate keys for origin, mixed and negative samples remain `minecraft:taiga`, `minecraft:desert` and `minecraft:desert`, sample keys exist in inventory, weighted selection total weight remains `675`, candidate entry count remains `14`, candidate inventory is ready, selector input algorithm is ready and selector input algorithm runtime is not ready, bridge implementation go/no-go decision is ready and bridge implementation go/no-go runtime is not ready, bridge readiness is ready and bridge readiness runtime is not ready, bridge go/no-go decision is ready and bridge go/no-go runtime is not ready, selector-input algorithm consolidation is ready and selector-input algorithm consolidation runtime is not ready, readiness-chain consolidation is ready and readiness-chain runtime is not ready, all current readiness layers are ready, runtime-ready layer count remains `0` and consolidated readiness layer count remains `6`, holder conversion readiness is ready and holder conversion runtime is not ready, holder resolution is not ready, resource-location conversion is not ready and resource-key conversion is not ready, possible-biomes readiness is ready and possible-biomes runtime is not ready, fallback policy readiness is ready and fallback policy runtime is not ready, fallback legacy biome name remains `PLAINS`, fallback candidate modern biome key remains `minecraft:plains`, codec method shape stub is ready and codec method shape runtime is not ready, designated subclass remains ready and designated subclass runtime is not ready, runtime holder return is not ready, collectPossibleBiomes implementation is not ready, getNoiseBiome implementation is not ready, usable runtime biome source is not ready, registry lookup access is not ready, usable codec implementation is not ready, codec is not registered and biome-source type is not registered, normal runtime construction is not allowed, unsupported method stubs only remain in place, selected-surface readiness item count remains `68`, global readiness matrix total requirement count remains `46`, global readiness matrix blocked requirement count remains `46`, dimension JSON is absent, dimension type JSON is absent, Caveman remains deferred, `CAVENIA_LEVEL_KEY` still resolves to `cavernreborn:cavenia`, `level.getServer().getLevel(CAVENIA_LEVEL_KEY) == null`, no `dimension/cavenia.json` or `dimension_type/cavenia.json` exists, no Cavenia configured or placed feature resources, biome modifiers, biome tags or other worldgen resources exist, direct Cavenic and crazy boundaries remain unchanged, and the selector-to-weighted-candidate bridge consolidation note at `docs/cavenia-runtime-biome-source-selector-to-weighted-candidate-bridge-consolidation-next-decision-mvp.md` documents that inert path.

The same runtime smoke slice now also covers the Cavenia Runtime BiomeSource Candidate-Key-to-Holder Conversion Go/No-Go MVP: `CaveniaRuntimeBiomeSourceCandidateKeyToHolderConversionGoNoGoDecision` now resolves from `app-neoforge`, selected decision is `PROCEED_WITH_CANDIDATE_KEY_TO_HOLDER_CONVERSION_READINESS_NEXT`, decision is go for candidate-key-to-holder conversion readiness next and remains readiness-only for the next slice, candidate-key-to-holder conversion readiness is not implemented in this slice, candidate-key-to-holder conversion implementation is not ready and candidate-key-to-holder conversion runtime is not ready, guardrail count is `20`, all guardrails are enforced in this slice, allowed next-slice readiness action count is `7`, allowed next-slice conversion implementation action count is `0`, allowed next-slice runtime action count is `0`, the next slice may add candidate-key-to-holder conversion readiness and may pin string candidate key surface, resource-location policy, resource-key policy, registry lookup policy, holder resolution policy and missing-or-fallback policy, while the next slice may not implement candidate-key-to-holder conversion, use registry lookup access, resolve holders, implement resource-location conversion or resource-key conversion, wire into `getNoiseBiome(...)`, or make `getNoiseBiome(...)` usable, selector-to-weighted-candidate bridge consolidation is ready and selector-to-weighted-candidate bridge consolidation runtime is not ready, selector-to-weighted-candidate bridge is ready and selector-to-weighted-candidate bridge runtime is not ready, candidate key output shape remains `string modern biome candidate key`, candidate keys remain string-only, candidate entry count remains `14`, candidate inventory is ready, sample candidate keys remain `minecraft:taiga`, `minecraft:desert` and `minecraft:desert`, sample keys exist in inventory, holder conversion readiness is ready and holder conversion runtime is not ready, holder resolution is not ready, resource-location conversion is not ready, resource-key conversion is not ready and registry lookup access is not ready, fallback policy readiness is ready and fallback policy runtime is not ready, fallback legacy biome name remains `PLAINS`, fallback candidate modern biome key remains `minecraft:plains`, possible-biomes readiness is ready and possible-biomes runtime is not ready, noise-biome selection readiness is ready and noise-biome selection runtime is not ready, readiness-chain consolidation is ready and readiness-chain runtime is not ready, all current readiness layers are ready, runtime-ready layer count remains `0` and consolidated readiness layer count remains `6`, codec method shape stub is ready and codec method shape runtime is not ready, designated subclass remains ready and designated subclass runtime is not ready, runtime holder return is not ready, collectPossibleBiomes implementation is not ready, getNoiseBiome implementation is not ready, usable runtime biome source is not ready, usable codec implementation is not ready, codec is not registered and biome-source type is not registered, normal runtime construction is not allowed, unsupported method stubs only remain in place, selected-surface readiness item count remains `68`, global readiness matrix total requirement count remains `46`, global readiness matrix blocked requirement count remains `46`, dimension JSON is absent, dimension type JSON is absent, Caveman remains deferred, `CAVENIA_LEVEL_KEY` still resolves to `cavernreborn:cavenia`, `level.getServer().getLevel(CAVENIA_LEVEL_KEY) == null`, no `dimension/cavenia.json` or `dimension_type/cavenia.json` exists, no Cavenia configured or placed feature resources, biome modifiers, biome tags or other worldgen resources exist, direct Cavenic and crazy boundaries remain unchanged, and the candidate-key-to-holder conversion go/no-go note at `docs/cavenia-runtime-biome-source-candidate-key-to-holder-conversion-go-no-go-mvp.md` documents that inert path.
The same runtime smoke slice now also covers the Cavenia Runtime BiomeSource Candidate-Key-to-Holder Conversion Readiness MVP: `CaveniaRuntimeBiomeSourceCandidateKeyToHolderConversionReadiness` now resolves from `app-neoforge`, candidate-key-to-holder conversion readiness is ready, string candidate key input is pinned, candidate key inventory source is pinned, resource-location policy is pinned, resource-key policy is pinned, biome registry lookup policy is pinned, holder resolution policy is pinned, holder return boundary is pinned, missing candidate key policy is pinned, unresolved resource-location policy is pinned, unresolved resource-key policy is pinned and missing holder fallback policy is pinned, conversion implementation is not ready, registry lookup is required for runtime conversion, registry lookup access is not ready, holder resolution is not ready, holder return is not ready, resource-location conversion is not ready, resource-key conversion is not ready and fallback runtime is not ready, real candidate-key-to-holder conversion is still deferred, candidate key input shape remains `string modern biome candidate key`, selector-to-weighted-candidate bridge consolidation is ready, selector-to-weighted-candidate bridge is ready and candidate keys remain string-only, candidate entry count remains `14`, sample candidate keys remain `minecraft:taiga`, `minecraft:desert` and `minecraft:desert`, sample keys exist in inventory, go/no-go decision is ready and go/no-go runtime is not ready, holder conversion readiness is ready and holder conversion runtime is not ready, fallback policy readiness is ready and fallback policy runtime is not ready, possible-biomes readiness is ready and possible-biomes runtime is not ready, noise-biome selection readiness is ready and noise-biome selection runtime is not ready, readiness-chain consolidation is ready and readiness-chain runtime is not ready, all current readiness layers are ready, runtime-ready layer count remains `0` and consolidated readiness layer count remains `6`, codec method shape stub is ready and codec method shape runtime is not ready, designated subclass remains ready and designated subclass runtime is not ready, runtime holder return is not ready, collectPossibleBiomes implementation is not ready, getNoiseBiome implementation is not ready, usable runtime biome source is not ready, usable codec implementation is not ready, codec is not registered and biome-source type is not registered, normal runtime construction is not allowed, unsupported method stubs only remain in place, selected-surface readiness item count remains `68`, global readiness matrix total requirement count remains `46`, global readiness matrix blocked requirement count remains `46`, dimension JSON is absent, dimension type JSON is absent, Caveman remains deferred, `CAVENIA_LEVEL_KEY` still resolves to `cavernreborn:cavenia`, `level.getServer().getLevel(CAVENIA_LEVEL_KEY) == null`, no `dimension/cavenia.json` or `dimension_type/cavenia.json` exists, no Cavenia configured or placed feature resources, biome modifiers, biome tags or other worldgen resources exist, direct Cavenic and crazy boundaries remain unchanged, and the candidate-key-to-holder conversion readiness note at `docs/cavenia-runtime-biome-source-candidate-key-to-holder-conversion-readiness-mvp.md` documents that inert path.
The same runtime smoke slice now also covers the Cavenia Runtime BiomeSource Candidate-Key-to-Holder Conversion Implementation Go/No-Go MVP: `CaveniaRuntimeBiomeSourceCandidateKeyToHolderConversionImplementationGoNoGoDecision` now resolves from `app-neoforge`, selected decision is `PROCEED_WITH_GUARDED_CANDIDATE_KEY_TO_HOLDER_CONVERSION_IMPLEMENTATION_NEXT`, decision is go for guarded candidate-key-to-holder conversion implementation next, candidate-key-to-holder conversion is not implemented in this slice, guardrail count is `16`, all guardrails are enforced in this slice, allowed next-slice conversion action count is `7`, allowed next-slice runtime wiring action count is `0`, allowed next-slice activation action count is `0`, the next slice may add guarded conversion helper, may use string candidate key input, may parse resource locations, construct resource keys, use biome registry lookup, resolve holders and handle missing-or-fallback only in the designated helper, and may not wire into `getNoiseBiome(...)`, make `getNoiseBiome(...)` usable, wire into `collectPossibleBiomes()`, make `collectPossibleBiomes()` usable, register codec, register biome-source type or register generator, candidate-key-to-holder conversion readiness is ready, conversion implementation is not ready, registry lookup access is not ready, holder resolution is not ready, holder return is not ready, resource-location conversion is not ready, resource-key conversion is not ready and fallback runtime is not ready, candidate key input shape remains `string modern biome candidate key`, candidate keys remain string-only, sample candidate keys remain `minecraft:taiga`, `minecraft:desert` and `minecraft:desert`, holder conversion readiness is ready, fallback policy readiness is ready, possible-biomes readiness is ready, noise-biome selection readiness is ready, readiness-chain consolidation is ready, all current readiness layers are ready, runtime-ready layer count remains `0` and consolidated readiness layer count remains `6`, codec method shape stub is ready, designated subclass is ready, runtime holder return is not ready, collectPossibleBiomes implementation is not ready, getNoiseBiome implementation is not ready, usable runtime biome source is not ready, codec is not registered, biome-source type is not registered, normal runtime construction is not allowed, unsupported method stubs only remain in place, selected-surface readiness item count remains `68`, global readiness matrix total requirement count remains `46`, global readiness matrix blocked requirement count remains `46`, dimension JSON is absent, dimension type JSON is absent, Caveman remains deferred, `CAVENIA_LEVEL_KEY` still resolves to `cavernreborn:cavenia`, `level.getServer().getLevel(CAVENIA_LEVEL_KEY) == null`, no `dimension/cavenia.json`, `dimension_type/cavenia.json`, Cavenia configured or placed feature resources, biome modifiers, biome tags or other worldgen resources exist, direct Cavenic and crazy boundaries remain unchanged, and the candidate-key-to-holder conversion implementation go/no-go note at `docs/cavenia-runtime-biome-source-candidate-key-to-holder-conversion-implementation-go-no-go-mvp.md` documents that inert path.
The same runtime smoke slice now also covers the Cavenia Runtime BiomeSource Candidate-Key-to-Holder Conversion Guarded Implementation MVP: `CaveniaRuntimeBiomeSourceCandidateKeyToHolderConverter` now resolves from `app-neoforge`, converter is ready, `holderForCandidateKey("minecraft:taiga", lookupProvider)`, `holderForCandidateKey("minecraft:desert", lookupProvider)` and `holderForCandidateKey("minecraft:plains", lookupProvider)` return present holders when the GameTest server provides biome lookup through `level.registryAccess()`, `holderForCandidateKey("not a valid key", lookupProvider)` and `holderForCandidateKey("cavernreborn:missing_biome", lookupProvider)` return empty, `holderForCandidateKeyOrFallback("cavernreborn:missing_biome", lookupProvider)` returns present when fallback `minecraft:plains` is resolvable, converter is not wired into `getNoiseBiome(...)`, converter is not wired into `collectPossibleBiomes()`, `getNoiseBiome(...)`, `collectPossibleBiomes()` and `codec()` remain stubbed, no Cavenia runtime level exists, no dimension JSON/type JSON exists, no worldgen/access/spawn resources exist, direct Cavenic and crazy boundaries remain unchanged, and the guarded converter implementation note at `docs/cavenia-runtime-biome-source-candidate-key-to-holder-conversion-guarded-implementation-mvp.md` documents that inert path.
The same runtime smoke slice now also covers the Cavenia Runtime BiomeSource Candidate-Key-to-Holder Converter Consolidation / Next Decision MVP: `CaveniaRuntimeBiomeSourceCandidateKeyToHolderConverterConsolidation` now resolves from `app-neoforge`, converter consolidation is ready, the designated converter exists, converter is ready, `holderForCandidateKey(...)` is ready, `holderForCandidateKeyOrFallback(...)` is ready, optional holder result is ready, fallback holder attempt is ready, server global access is absent, `getNoiseBiome(...)` wiring is not ready, `collectPossibleBiomes()` wiring is not ready, runtime biome source is not ready, the selected next decision is `PROCEED_WITH_COLLECT_POSSIBLE_BIOMES_HOLDER_SET_GO_NO_GO_NEXT`, collectPossibleBiomes holder-set go/no-go is next, holder-set readiness, implementation and runtime are not ready, the next slice may add only collectPossibleBiomes holder-set go/no-go decision, may not add holder-set readiness, may not implement collectPossibleBiomes, may not wire converter into collectPossibleBiomes or getNoiseBiome, the guarded converter still resolves `minecraft:taiga`, `minecraft:desert` and `minecraft:plains` through caller-provided lookup, invalid and missing candidate keys still return empty, fallback still resolves fallback when available, `getNoiseBiome(...)`, `collectPossibleBiomes()` and `codec()` remain stubbed, no Cavenia runtime level exists, no dimension JSON/type JSON exists, no worldgen/access/spawn resources exist, direct Cavenic and crazy boundaries remain unchanged, and the converter consolidation next-decision note at `docs/cavenia-runtime-biome-source-candidate-key-to-holder-converter-consolidation-next-decision-mvp.md` documents that inert path.
The same runtime smoke slice now also covers the Cavenia Runtime BiomeSource Collect-Possible-Biomes Holder Set Go/No-Go MVP: `CaveniaRuntimeBiomeSourceCollectPossibleBiomesHolderSetGoNoGoDecision` now resolves from `app-neoforge`, selected decision is `PROCEED_WITH_COLLECT_POSSIBLE_BIOMES_HOLDER_SET_READINESS_NEXT`, decision is readiness-only for next slice, collectPossibleBiomes holder-set readiness is not implemented in this slice, collectPossibleBiomes holder-set implementation is not ready, collectPossibleBiomes holder-set runtime is not ready, guardrail count is `18`, all guardrails are enforced in this slice, allowed next-slice readiness action count is `9`, allowed next-slice implementation action count is `0`, allowed next-slice runtime wiring action count is `0`, the next slice may add collectPossibleBiomes holder-set readiness, may not implement collectPossibleBiomes, may not wire converter into collectPossibleBiomes and may not wire converter into getNoiseBiome, converter consolidation is ready, converter is ready, `holderForCandidateKey(...)` and `holderForCandidateKeyOrFallback(...)` are ready, `holderForCandidateKey("minecraft:taiga", lookupProvider)`, `holderForCandidateKey("minecraft:desert", lookupProvider)` and `holderForCandidateKey("minecraft:plains", lookupProvider)` still resolve present holders when the GameTest server provides biome lookup through `level.registryAccess()`, invalid and missing candidate keys still return empty, fallback still resolves fallback when available, `getNoiseBiome(...)`, `collectPossibleBiomes()` and `codec()` remain stubbed, no Cavenia runtime level exists, no dimension JSON/type JSON exists, no worldgen/access/spawn resources exist, direct Cavenic and crazy boundaries remain unchanged, and the collect-possible-biomes holder-set go/no-go note at `docs/cavenia-runtime-biome-source-collect-possible-biomes-holder-set-go-no-go-mvp.md` documents that inert path.
The same runtime smoke slice now also covers the Cavenia Runtime BiomeSource Collect-Possible-Biomes Holder Set Readiness MVP: `CaveniaRuntimeBiomeSourceCollectPossibleBiomesHolderSetReadiness` now resolves from `app-neoforge`, collect-possible-biomes holder-set readiness is ready, candidate inventory source is pinned, converter dependency is pinned, lookup provider requirement is pinned, pre-resolved holder-set policy is pinned, holder-set output shape is pinned, stream return boundary is pinned, deduplication policy is pinned, ordering policy is pinned, fallback-if-empty policy is pinned, missing/invalid/unresolved handling policies are pinned, collectPossibleBiomes implementation is not ready, converter wiring in runtime methods is not ready, getNoiseBiome wiring is not ready, runtime construction/factory is not ready, converter is ready, `holderForCandidateKey(...)` and `holderForCandidateKeyOrFallback(...)` are ready, `holderForCandidateKey("minecraft:taiga", lookupProvider)`, `holderForCandidateKey("minecraft:desert", lookupProvider)` and `holderForCandidateKey("minecraft:plains", lookupProvider)` still resolve present holders when the GameTest server provides biome lookup through `level.registryAccess()`, invalid and missing candidate keys still return empty, fallback still resolves fallback if available, `getNoiseBiome(...)`, `collectPossibleBiomes()` and `codec()` remain stubbed, no Cavenia runtime level exists, no dimension JSON/type JSON exists, no worldgen/access/spawn resources exist, direct Cavenic and crazy boundaries remain unchanged, and the collect-possible-biomes holder-set readiness note at `docs/cavenia-runtime-biome-source-collect-possible-biomes-holder-set-readiness-mvp.md` documents that inert path.
The same runtime smoke slice now also covers the Cavenia Runtime BiomeSource Collect-Possible-Biomes Holder Set Implementation Go/No-Go MVP: `CaveniaRuntimeBiomeSourceCollectPossibleBiomesHolderSetImplementationGoNoGoDecision` now resolves from `app-neoforge`, selected decision is `PROCEED_WITH_GUARDED_COLLECT_POSSIBLE_BIOMES_HOLDER_SET_BUILDER_NEXT`, decision is implementation-only for next slice, collectPossibleBiomes holder-set builder is not implemented in this slice, collectPossibleBiomes holder-set runtime is not ready, guardrail count is `18`, all guardrails are enforced in this slice, allowed next-slice builder action count is `8`, allowed next-slice runtime wiring action count is `0`, allowed next-slice activation action count is `0`, the next slice may add guarded pre-resolved holder-set builder, may use candidate inventory input, may use designated converter dependency, may require lookup provider parameter, may not wire builder into collectPossibleBiomes, may not implement collectPossibleBiomes, may not wire builder into getNoiseBiome and may not make runtime methods usable, collectPossibleBiomes holder-set readiness is ready, converter is ready, `holderForCandidateKey(...)` and `holderForCandidateKeyOrFallback(...)` are ready, `holderForCandidateKey("minecraft:taiga", lookupProvider)`, `holderForCandidateKey("minecraft:desert", lookupProvider)` and `holderForCandidateKey("minecraft:plains", lookupProvider)` still resolve present holders when the GameTest server provides biome lookup through `level.registryAccess()`, invalid and missing candidate keys still return empty, fallback still resolves fallback if available, `getNoiseBiome(...)`, `collectPossibleBiomes()` and `codec()` remain stubbed, no Cavenia runtime level exists, no dimension JSON/type JSON exists, no worldgen/access/spawn resources exist, direct Cavenic and crazy boundaries remain unchanged, and the collect-possible-biomes holder-set implementation go/no-go note at `docs/cavenia-runtime-biome-source-collect-possible-biomes-holder-set-implementation-go-no-go-mvp.md` documents that inert path.
The same runtime smoke slice now also covers the Cavenia Runtime BiomeSource Collect-Possible-Biomes Holder Set Guarded Builder MVP: `CaveniaRuntimeBiomeSourceCollectPossibleBiomesHolderSetBuilder` now resolves from `app-neoforge`, builder is ready, `buildCandidateHolderList(...)` is ready, `buildCandidateHolderList(lookupProvider)` returns a non-empty immutable holder list when the GameTest server provides biome lookup through `level.registryAccess()`, duplicate candidate key inputs are deduplicated, invalid-or-missing-only candidate inputs fall back to `minecraft:plains` when fallback resolves, empty candidate input also falls back to `minecraft:plains` when fallback resolves, null lookup/provider returns an empty immutable list and does not throw, the builder is deterministic across repeated calls, the builder is not wired into `collectPossibleBiomes()` or `getNoiseBiome(...)`, `getNoiseBiome(...)`, `collectPossibleBiomes()` and `codec()` remain stubbed, no Cavenia runtime level exists, no dimension JSON/type JSON exists, no worldgen/access/spawn resources exist, direct Cavenic and crazy boundaries remain unchanged, and the guarded builder note at `docs/cavenia-runtime-biome-source-collect-possible-biomes-holder-set-guarded-builder-mvp.md` documents that bounded helper path.
The same runtime smoke slice now also covers the Cavenia Runtime BiomeSource Runtime Construction / Factory Go/No-Go MVP: `CaveniaRuntimeBiomeSourceRuntimeConstructionFactoryGoNoGoDecision` now resolves from `app-neoforge`, selected decision is `PROCEED_WITH_RUNTIME_CONSTRUCTION_FACTORY_READINESS_NEXT`, decision is readiness-only for the next slice, runtime construction/factory readiness is not implemented in this slice, runtime construction/factory implementation and runtime readiness remain blocked, guardrail count is `22`, all guardrails remain enforced, allowed next-slice readiness action count is `9`, implementation and runtime-wiring counts remain `0`, the next slice may add runtime construction/factory readiness data and may pin private-constructor policy, factory-method policy, pre-resolved holder-state policy, holder-set builder dependency, lookup/provider input policy, immutable state-storage policy, collectPossibleBiomes stream-source policy and no-activation boundary, while it still may not implement runtime construction/factory, add a public constructor, add a factory method, add pre-resolved holder state, wire the builder into runtime methods, make runtime methods usable, or open registration or activation surfaces, builder consolidation remains ready, the builder remains ready and unwired, the builder still returns non-empty lists through server-provided lookup/provider, `getNoiseBiome(...)`, `collectPossibleBiomes()` and `codec()` remain stubbed, no Cavenia runtime level exists, no dimension JSON/type JSON exists, no worldgen/access/spawn resources exist, and the runtime construction/factory go/no-go note at `docs/cavenia-runtime-biome-source-runtime-construction-factory-go-no-go-mvp.md` documents that inert path.
