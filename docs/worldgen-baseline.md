...
- Resource generation is mining-oriented by default:
  ...
  - the second special-ore slice adds `hexcite`, `randomite` and `fissured_stone` through the same narrow checked-in worldgen path;
  ...
  - `randomite` is wired into `stone_depths`, `lush_grotto` and `highland_hollows`;
  - `hexcite` is wired into a deeper dry band plus a rarer upper highland pass;
  - `fissured_stone` is wired into `stone_depths` and `dripstone_grotto` as a local cave-effect block, not as a destructive hazard;
...
- Special-ore/content design note: `docs/cavern-special-ore-content-parity.md`
...
- The old custom ores, tower dungeons, mirage remnants, Huge Cavern and Aqua Cavern are still out of scope beyond the currently checked-in bounded slices.
- The current ore/content work restores `aquamarine`, `magnite`, `hexcite`, `randomite` and `fissured_stone`; the wider legacy ore roster, destructive fissure behavior, full vein-provider parity and broader special-case mechanics are still out of scope.
