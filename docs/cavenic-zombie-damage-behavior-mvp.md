# Cavenic Zombie Legacy Damage Behavior MVP

This note documents the bounded damage-behavior follow-up for the existing `cavernreborn:cavenic_zombie`.

It does not add a new mob, new AI, new spawn rules, new loot rules or broader Cavenic combat systems. It only restores the small confirmed legacy damage drift from `EntityCavenicZombie`.
...
Legacy `EntityCavenicZombie` handled damage like this:
...
- Reborn implements the behavior directly in `app.entity.CavenicZombie`.
...
- vanilla zombie AI remains unchanged.
- attributes, natural spawning and orb-drop behavior remain unchanged.

## Why This Mapping Is Safe

- The legacy behavior lived directly on `EntityCavenicZombie` rather than in a broad shared system.
- Reborn keeps the implementation entity-specific instead of using a global zombie or mob damage event.
...
- NeoForge GameTest runtime smoke covers:
  ...
  - continued vanilla zombie loot-table baseline and orb-drop follow-up stability
