# Crazy Spider Particle Trail MVP

This note documents the bounded client-visual particle follow-up for the existing `cavernreborn:crazy_spider`.

It restores only the legacy Crazy Spider particle trail. It does not change natural spawning, loot, incoming damage, boss bar / sky-darkening, custom combat, blindness/poison behavior, custom AI, packets or broader crazy-mob rendering systems.
...
- legacy client-only `onUpdate()` on `EntityCrazySpider`
...
- `EntityCrazySpider#onUpdate()` is client-only through `@SideOnly(Side.CLIENT)`.
...
- This particle branch matches Crazy Zombie, Crazy Skeleton and Crazy Creeper exactly.
- No packets are involved.
- There is no dedicated legacy particle texture asset in the adjacent `../cavern-2` resources.
- Boss bar / sky-darkening, incoming damage and direct blindness/poison combat behavior are separate branches and do not drive the particle trail.
...
- The shared particle class stays `CrazyMobParticle`.
...
- Because the legacy particle reused the portal base and no dedicated crazy texture asset was found, Reborn continues to reuse the vanilla portal sprite set through:
  ...
  - `minecraft:generic_6`
  - `minecraft:generic_7`
- No second Crazy Spider-specific particle id, provider or texture set is added.

## Exact Reborn Entity Hook

- Reborn keeps `CrazySpider extends Spider`.
- Reborn restores the trail through `CrazySpider#aiStep()`.
...
- Boss bar is documented in `docs/crazy-spider-boss-bar-mvp.md`.
- Damage remains unchanged.
- Damage is documented in `docs/crazy-spider-damage-behavior-mvp.md`.
...
- Resource tests pin:
  ...
  - `CrazySpider#aiStep()` and its client-only guard
