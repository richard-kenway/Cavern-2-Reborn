# Cavenic Skeleton Legacy Damage Behavior MVP

This note documents the bounded damage-behavior follow-up for the existing `cavernreborn:cavenic_skeleton`.

It does not add a new mob, custom AI, custom arrows, new spawn rules or new loot rules. It only restores the small confirmed legacy fall/fire damage drift from `EntityCavenicSkeleton`.
...
Legacy `EntityCavenicSkeleton` handled damage like this:
...
- Reborn implements the behavior directly in `app.entity.CavenicSkeleton`.
...
- vanilla skeleton AI remains unchanged.
- attributes, natural spawning and `1/5` orb-drop behavior remain unchanged.

## Why This Mapping Is Safe

- The legacy behavior lived directly on `EntityCavenicSkeleton` rather than in a broad shared system.
- Reborn keeps the implementation entity-specific instead of using a global skeleton or mob damage event.
...
- NeoForge GameTest runtime smoke covers:
  ...
  - continued vanilla skeleton loot-table baseline, `1/5` orb-drop follow-up and natural-spawn stability
