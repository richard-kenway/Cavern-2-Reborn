# Cavenic Zombie Natural Spawn MVP

This note documents the bounded natural-spawn follow-up for the existing `cavenic_zombie` baseline mob.

It does not add a new mob. It only makes the already registered `cavernreborn:cavenic_zombie` spawn naturally inside the existing `CAVERN` gameplay context.
...
- This keeps the block/light logic zombie-like while hard-gating the mob to the `CAVERN` dimension.

## Biome Modifier Targeting

- Reborn uses a NeoForge biome modifier at `data/cavernreborn/neoforge/biome_modifier/cavenic_zombie_spawns.json`.
- The modifier type is `neoforge:add_spawns`.
- It targets the narrow biome tag `cavernreborn:spawns_cavenic_zombie`.
...
- Legacy `cavern.entity.CaveEntityRegistry` added `EntityCavenicZombie` to the shared Cavenic spawn list with:
  - weight: `30`
  - group size: `2..2`
...
- Resource tests cover:
  ...
  - the `cavernreborn:spawns_cavenic_zombie` biome tag
  - legacy weight/group-size pinning
  - the no-extra-mobs / no-custom-loot / no-Cavenia boundary
...
- `cavenic_orb` mob drops remain out of scope
