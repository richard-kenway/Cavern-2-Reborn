# Cavenic Skeleton Natural Spawn MVP

This note documents the bounded natural-spawn follow-up for the existing `cavenic_skeleton` baseline mob.

It does not add a new mob. It only makes the already registered `cavernreborn:cavenic_skeleton` spawn naturally inside the existing `CAVERN` gameplay context.
...
- This keeps the block/light logic skeleton-like while hard-gating the mob to the `CAVERN` dimension.

## Biome Modifier Targeting

- Reborn uses a NeoForge biome modifier at `data/cavernreborn/neoforge/biome_modifier/cavenic_skeleton_spawns.json`.
- The modifier type is `neoforge:add_spawns`.
- It targets the narrow biome tag `cavernreborn:spawns_cavenic_skeleton`.
- That biome tag is defined at `data/cavernreborn/tags/worldgen/biome/spawns_cavenic_skeleton.json`.
...
- Legacy `cavern.entity.CaveEntityRegistry` added `EntityCavenicSkeleton` to the shared Cavenic spawn list with:
  - weight: `20`
  - group size: `1..1`
...
- Resource tests cover:
  ...
  - the `cavernreborn:spawns_cavenic_skeleton` biome tag
  - legacy weight/group-size pinning
  - the no-custom-loot / no-custom-arrow / no-Cavenia boundary
...
- `cavenic_orb` skeleton drops remain out of scope
